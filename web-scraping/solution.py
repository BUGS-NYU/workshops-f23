...
def get_column_names(header_columns: Locator):
    return [cleanTextContent(header_columns.nth(i).text_content()) for i in range(header_columns.count())]
...
def scrape_course(course: Locator, column_names: List[str]):
    ...
    # children of?
    descriptions = course.locator("span")
    for i, column_name in enumerate(column_names):
        ...
        description_text = cleanTextContent(description.text_content())
...
def scrape_courses(courses: Locator, column_names: List[str]):
    return [scrape_course(courses.nth(i), column_names) for i in range(courses.count())]
